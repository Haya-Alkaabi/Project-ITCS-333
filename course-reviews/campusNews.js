/* 
##########################################################################################
i'm facing an issue ----- i can't link Replit to the grpoup's repository. and when i
  put the replit code in VSCode, i face problems with the fetch. So i commented the
 Replit code (the correct one)here 
 ##########################################################################################

 replit link: https://26228faa-d6f3-47ae-8092-cf4002fee263-00-311whzuj3t2cr.sisko.replit.dev/Project-ITCS-333/course-reviews/CampusNews.html
 */
...
class CampusNews {
  constructor() {
      this.apiUrls = [
          "https://680be6c92ea307e081d2a476.mockapi.io/Campus-News",
          "/Project-ITCS-333/course-reviews/get_articles.php"
      ];
      this.articles = [];
  ...
  async loadData() {
      try {
          ...
          this.articles = this.processArticles(combinedData);
  ...
  processArticles(articles) {
      return articles.map(article => ({
          id: article.id,
          title: article.title,
          summary: article.content ? article.content.substring(0, 200) + '...' : article.summary,
          category: article.category,
          image: article.image_path || article.image,
          author: article.author,
          publishDate: new Date(article.publishDate || article.publish_date).toISOString(),
  ...
  createArticleCard(article, isRecommended) {
      ...
      if (isRecommended) {
          ...
          card.innerHTML = `
              ...
              <a href="#" class="mt-auto block px-4 py-2 bg-red text-white text-center rounded hover:bg-gold transition-colors">Read more</a>
          `;
      } else {
          ...
          card.innerHTML = `
              ...
              <div class="flex items-center gap-4 mt-4 md:mt-0">
                  <img src="${article.image}" alt="${article.title}" 
                       ...
                       onerror="this.onerror=null;this.src='../photos/uob.png'">
                  <a href="#" class="px-4 py-2 bg-gold text-white rounded hover:bg-red transition-colors whitespace-nowrap">Read</a>
              </div>
          `;
      }
      return card;
...
//phase 2 code (with fetch data from mock api)
class CampusNews {
    constructor() {
